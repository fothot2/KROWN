#!/usr/bin/env python3
"""Execute the BSBM QLever matrix stage."""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Execute the BSBM QLever system."
    )
    parser.add_argument(
        "--scenario",
        type=Path,
...
def execute_qlever_stage(
    scenario: Path,
    declaration: Path,
    verbose: bool = True,
) -> bool:
    """Execute QLever through the generic matrix resource."""
    ...
    return resource.execute(
        ...
        results_file="raw/bsbm-qlever-summary.json",
        output_file="raw/bsbm-qlever-results.tar.gz",
        selected_systems=list(QLEVER_SYSTEMS),
        failure_results_file="raw/bsbm-qlever-failed-summary.json",
        failure_output_file="raw/bsbm-qlever-failed-results.tar.gz",
    )


def main() -> int:
    arguments = parse_arguments()
    success = execute_qlever_stage(
        arguments.scenario,
