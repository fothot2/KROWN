...
class QLeverRequestCapProvenanceTests(unittest.TestCase):
    ...
    def test_rdflib_record_omits_http_request_cap_fields(self):
        record = self.record()
        record["measurement_boundary"] = "rdflib-full-result-materialization"
        del record["request_max_rows"]
        del record["result_cap_requested"]
        compact = _compact_result_record(record)
        self.assertEqual(
            compact["measurement_boundary"],
            "rdflib-full-result-materialization",
        )
        self.assertNotIn("request_max_rows", compact)
        self.assertNotIn("result_cap_requested", compact)
