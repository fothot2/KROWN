...
class CampaignTests(unittest.TestCase):
 ...
 def test_outcome_counts_separate_reportable_and_terminal_failures(self):
  ...
  self.assertEqual(RdfCampaign._outcome_counts(rows),{"reportable_count":2,"structural_failure_count":1,"timed_out_count":1,"interrupted_count":1,"infrastructure_blocked_count":0,"confirmed_oom_count":0})
 def test_run_continues_after_failure_and_across_repetitions(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   with unittest.mock.patch(
    'bench_executor.rdf_campaign.require_clean_matrix_environment'
   ):
    result=campaign.run()
