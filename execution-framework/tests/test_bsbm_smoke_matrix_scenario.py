...
class BsbmSmokeMatrixScenarioTests(unittest.TestCase):
 ...
 def test_scenario_prepares_then_executes_external_matrix(self):
  self.assertEqual([step['resource'] for step in self.metadata['steps']],['RdfManifestResource','RdfExperimentMatrixResource','RdfExperimentMatrixResource'])
  self.assertEqual(self.metadata['steps'][1]['command'],'preflight')
  self.assertEqual(self.metadata['steps'][1]['parameters']['output_file'],'raw/bsbm-matrix-preflight.json')
  matrix=self.metadata['steps'][2]['parameters']
